use ckb_dao_cobuild_schemas::{
    Capacity, Claim, ClaimVec, DaoActionData, Deposit, DepositVec, Uint64, Withdraw, WithdrawVec,
};
use ckb_testtool::{
    ...
    ckb_types::{
        bytes::Bytes,
        core::{Cycle, HeaderView, TransactionBuilder, TransactionView},
        packed,
        prelude::*,
    },
...
pub fn pack_uint64(number: u64) -> Uint64 {
    Uint64::new_unchecked(Bytes::from(number.to_le_bytes().to_vec()))
}
...
impl TxSpec for DefaultTxSpec {
    ...
    fn new_tx_builder(
        ...
        TransactionBuilder::default()
            ...
            .outputs_data(vec![dao_output_spec.data, Bytes::new()].pack())
    ...
    F: Fn(&mut CustomTxSpec) -> Withdraw,
{
    ...
    spec.on_new_output_spec(|cell| CellSpec {
        data: Bytes::from(hex!("0100000000000000").to_vec()),
        ..cell
    });
